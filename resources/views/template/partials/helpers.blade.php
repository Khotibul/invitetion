{{--
    Template Helpers — include di @php block setiap template
    Menyediakan variabel siap pakai dari $data dan $other
--}}
@php
use Carbon\Carbon;
...
// ── Foto sampul — fix path (sistem simpan di storage/, bukan storage/cover/)
$coverObj = $data->cover->description->image ?? null;
$coverSrc = null;
if ($coverObj && !empty($coverObj->image)) {
    if (($coverObj->method ?? '') === 'asset') {
        $coverSrc = asset($coverObj->image);
    } elseif (($coverObj->method ?? '') === 'storage') {
        $coverSrc = url('storage/sm/'.$coverObj->image);
    } elseif (($coverObj->method ?? '') === 'avatar') {
        $coverSrc = url('storage/avatar/'.$coverObj->image);
    } else {
        $coverSrc = url('storage/'.$coverObj->image);
    }
}

// ── Invitation file (untuk OG image)
...
// ── Galeri
$galleryFiles = ($other['photo'] && !empty($other['photo']->prop->file ?? []))
