...
@php
    ...
    // Font yang sudah di-load oleh template (tidak perlu load ulang)
    $_builtinFonts = [
        'Caveat','Dancing Script','Great Vibes','Kaushan Script','Nova Cut',
        'Raleway','Righteous','Satisfy','Didact Gothic','Sacramento','Estonia',
        'Oswald','Yanone Kaffeesatz','Merriweather','Courgette','Playfair Display',
        'Poppins','Lato','Cormorant Garamond','Pacifico','Amiri',
        'Fraunces','Plus Jakarta Sans',
    ];
    $_needLoadTitle   = $_fTitleFont   && !in_array($_fTitleFont,   $_builtinFonts);
    $_needLoadContent = $_fContentFont && !in_array($_fContentFont, $_builtinFonts);
...
@if($_needLoadTitle)
<link href="https://fonts.googleapis.com/css2?family={{ str_replace(' ', '+', $_fTitleFont) }}&display=swap" rel="stylesheet">
@endif
@if($_needLoadContent && $_fContentFont !== $_fTitleFont)
<link href="https://fonts.googleapis.com/css2?family={{ str_replace(' ', '+', $_fContentFont) }}&display=swap" rel="stylesheet">
@endif
...
:root { {{ $_rootVars }} }
@if($_fTitleFont)
h1,h2,h3,.cover-names,.couple-names,.hero h1,.hero-logo,
